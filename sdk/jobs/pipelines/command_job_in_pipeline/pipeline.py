...
train_job = CommandJob(
    inputs=dict(
        training_data=InputDatasetEntry(
            dataset=Dataset(local_path=parent_dir + "./data/")
        ),
        max_epocs=20,
        learning_rate=1.8,
        learning_rate_schedule="time-based",
    ),
    outputs=dict(model_output=None),
...
eval_job = CommandJob(
    inputs=dict(
        scoring_result=InputDatasetEntry(
            dataset=Dataset(local_path=parent_dir + "./data/")
        ),
    ),
    outputs=dict(eval_output=None),
